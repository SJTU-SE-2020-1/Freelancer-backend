...
public interface WorkRepository extends JpaRepository<Work, Integer> {
    ...
    @Query(value = "select w from Work w where w.status!=-1 and w.u_id = ?1 and w.title like %?2% and w.paymentHigher < ?3 and w.paymentLower > ?4")
    Page<Work> getPostedWorks(Integer uId, String keyword, Double paymentHigher, Double paymentLower,
            Pageable pageable);
